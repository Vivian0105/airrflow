process PRESTO_FILTERSEQ_MASKQUAL {
    ...
    output:
    tuple val(meta), path("*maskqual-pass.fastq") ,  emit: reads
    ...
    """
    FilterSeq.py maskqual -s $reads \\
        $args \\
        --outname ${meta.id} --log ${reads.baseName}.log --nproc ${task.cpus} > ${meta.id}_maskqual_command_log.txt
